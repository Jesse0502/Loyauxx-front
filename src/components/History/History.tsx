...
function TopArtists() {
	...
	useEffect(() => {
		let scrollToTop = history.listen(() => {
			window.scrollTo(0, 0);
		});
		return () => {
			scrollToTop();
	...
	return (
		<Box bg="mainLight">
			<Center h="12" justifyContent={"flex-end"} px="5">
				...
				<Box mt="-48" zIndex="0" w={{ lg: "container.lg" }}>
					<Flex justifyContent={"space-between"} color="white">
						...
						<Text
							...
							Listen them on the loyauxx playlist on spotify
						</Text>
					</Flex>
					<Accordion defaultIndex={[0]} allowMultiple>
						<Box rounded="lg">
							{Array.from(Array(showMore ? 5 : 10)).map((a, b) => (
								<motion.div
									initial="hidden"
									animate="visible"
									variants={variants}>
			...
			<Center w="container.lg" m="auto">
				<Stack>
					<Heading>Follow the Artists</Heading>
